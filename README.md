...
- **Books**: *Sombras, Datos y Relampagos*, *La Sospecha Razonable*, *No apagues la luz* (horror, 11 stories + prologue, full bilingual web edition: 13 EN chapters + 13 ES chapters), *Huella*, *La Necesidad de Creer*, and *Manual de bolsillo para sonidistas* (technical sound engineering reference, 40pp, Spanish only). All 6 books now have individual pages in all 7 languages (EN, ES, FR, IT, RU, JA, KO = 42 book subpages). JA pages feature cultural localization connecting to Japanese literary traditions; KO pages feature cultural localization connecting to Korean cultural traditions (한, 민중미술, K-horror, 직지, 무속, 판소리). Available on [Amazon](https://www.amazon.com/stores/Ernesto-Cisneros-Cino/author/B0FP5KBFNN), Zenodo (open access), and via Stripe (PDF).
...
- **Fonts**: Cinzel (display), Cormorant Garamond (body), Space Mono (mono) via Google Fonts; Noto Serif JP for Japanese, Noto Serif KR for Korean
- **Korean typography**: `word-break: keep-all`, `line-break: strict`, Noto Serif KR
- **Japanese typography**: kinsoku shori (禁則処理), `line-break: strict`, Noto Serif JP
...
The site contains ~346 pages across 7 full languages + 7 additional translation languages. Always verify that schema, hreflang, and analytics changes are propagated to all language versions.
